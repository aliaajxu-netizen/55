...
let state = {
  ...
  ratings: {},
  mastery: {},
  expandedCards: { "p01-a01-i01": true }, // Question 1 open by default
  theme: "light",
  filter: "all"
};
...
function performReset() {
  ...
  state.expandedCards = { "p01-a01-i01": true };
  saveState();
  navigateTo("practice");
}
...
function renderHomeScreen(container) {
  ...
  const homeHTML = `
    <div class="home-screen" id="home-screen">
      <h1 class="home-title">الأسئلة الوزارية حول أسماء الاستفهام المجموعة الأولى (مَنْ - مَنْ ذا، ما - ماذا) لقواعد اللغة العربية للصف السادس الإعدادي</h1>
      ...
      <div class="home-steps-card">
        ...
        <ul class="home-steps-list">
          <li>
            ...
            <span class="home-steps-text">اكتب جوابك الشخصي كاملاً وبكل أمانة في الحقل المخصص.</span>
          </li>
          <li>
            <span class="home-steps-num">٢</span>
            <span class="home-steps-text">اضغط على زر (أظهر الجواب النموذجي) للمقارنة الدقيقة مع المصدر.</span>
          </li>
          <li>
            <span class="home-steps-num">٣</span>
            <span class="home-steps-text">قيّم جوابك يا بطل بموضوعية واختر الدرجة المناسبة من (0 إلى 10).</span>
          </li>
          <li>
            <span class="home-steps-num">٤</span>
            <span class="home-steps-text">حدّد مستوى تمكنك من السؤال لمراجعة نقاط ضعفك لاحقاً بكل سهولة.</span>
          </li>
        </ul>
      </div>
...
function renderPracticeScreenProgressOnly() {
  ...
  if (labels.length >= 2) {
    labels[0].innerHTML = `<span style="width: 8px; height: 8px; border-radius: 50%; background-color: var(--madrasati-medium-purple); display: inline-block; margin-left: 0.25rem;"></span>تقدمك في الحل: ${answeredCount} من ${totalQuestions} أسئلة`;
  ...
  if (rail) {
    ...
    QUESTIONS.forEach((q, idx) => {
      const dot = rail.querySelector(`.nav-dot[data-q-id="${q.id}"]`);
      if (dot) {
    ...
    badgeHTML = `
      <div class="achievement-badge-card ${badgeColorClass}">
        ...
        <div class="badge-image-container">
          <img src="${badgeImg}" alt="${badgeTitle}" class="badge-glowing-image" referrerPolicy="no-referrer" onerror="this.style.display='none';">
        </div>
